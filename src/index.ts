...
const deAnsi = (str: string) => str.replace(/\x1B\[\d+m/g, '');
...
async function looseResolve(cwd: string, file: string) {
...
async function buildAction(entry: string | undefined, opts: BuildArgs) {
	opts.entry = entry;
